...
export function IndexingRequestForm() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                toast.success("Indexing request submitted! You'll receive an email when complete.");
                setAddress("");
            } else {
        ...
        return (
            <Card className="max-w-md w-full">
                <CardHeader>
                    ...
                    <CardDescription>Sign in to index any dApp contract address</CardDescription>
                </CardHeader>
                <CardContent>
                    <p className="text-sm text-muted-foreground mb-4">
                        Get detailed analytics and insights for any Ethereum smart contract.
                    </p>
                    <Button variant="secondary" className="w-full" disabled>Sign in to Request</Button>
                </CardContent>
            </Card>
        );
    }
    ...
    return (
        <Card className="max-w-md w-full">
            <CardHeader>
                <CardTitle>Request Custom Indexing</CardTitle>
                <CardDescription>Enter an Ethereum contract address to index</CardDescription>
            </CardHeader>
