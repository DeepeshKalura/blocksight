...
export function DaoCard({ dao, previewMode = false }: DaoCardProps) {
  const statusInfo = getStatusInfo(dao.status);

  return (
    <Card
      className={`bg-card/50 border-border backdrop-blur-sm transition-all flex flex-col ${previewMode ? "opacity-70" : "hover:border-accent/50"}`}
    >
